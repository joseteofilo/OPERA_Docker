...
# OPERA Docker Wrapper Script (Parallel Version)
...
DOCKER_CMD="docker run --rm --platform linux/amd64 --shm-size=8g -m 16g"
ARGS=("$@")
INPUT_FILE=""
HAS_STD=false
...
if [ -n "$INPUT_FILE" ]; then
    ...
    for arg in "$@"; do
        ...
        if [ "$PREV_WAS_OUTPUT" = true ]; then
            PREV_WAS_OUTPUT=false
            OUTPUT_NAME=$(basename "$arg")
            FINAL_ARGS+=("/data/$OUTPUT_NAME")
            continue
        fi
    ...
    echo "Running OPERA (Parallel) on $FILE_NAME..."
