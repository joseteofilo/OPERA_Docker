<#
.SYNOPSIS
Wrapper script for OPERA Docker container (Parallel Version) on Windows PowerShell.
Usage: .\run_opera_par.ps1 -s <file> [options]
#>
...
$DockerArgs = @("run", "--rm", "--platform", "linux/amd64", "--shm-size=8g", "-m", "16g")
...
if ($InputFile) {
    ...
    for ($i = 0; $i -lt $args.Count; $i++) {
        ...
        # If previous arg was -o, rewrite this to /data/filename
        if ($PrevWasOutput) {
            $PrevWasOutput = $false
            $OutputName = Split-Path $arg -Leaf
            $FinalArgs += "/data/$OutputName"
            continue
        }

        # Check if this is the output flag
    ...
    Write-Host "Running OPERA (Parallel) on $FileName..."
